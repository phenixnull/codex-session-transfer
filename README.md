...
<p align="center">
  ...
  <img alt="Version" src="https://img.shields.io/badge/version-0.4.8-blue">
...
- **Paged previews** - the preview pane requests 24 plan items at a time and loads the next page only when you scroll.
- **Local safety checks** - show database integrity, WAL files, blocking ChatGPT/Codex processes, and session index status before write operations.
...
| Provider mirror backups | `~/.codex/session-transfer/backups/provider-mirror-*` |
...
Codex Session Transfer is designed as a local maintenance tool. It does not upload session data to an external service. Local provider mirroring is an explicit full-target replacement: it backs up every runtime database, the session index, and all affected rollout files before changing data. Preview is optional. When the app detects running ChatGPT, Codex, Codex++, or provider-switcher processes that may hold database locks, it blocks the write until they are closed.
